...
Historical simulated rows return `STRIPE_REFUND_UNVERIFIED` on normal idempotency replay until a distinct verified provider ledger row exists. Provider-initiated partial refunds enter the ledger as settled but unallocated; reporting must count the money and surface item-allocation uncertainty rather than inventing precision.

...
- removes those default grants for future objects created by the migration's database role (`postgres`). Supabase-managed owner default ACL rows may remain, but the schema-level denial prevents API access to objects they create.
